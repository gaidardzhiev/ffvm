...
[Stage 8](./ffvm_stage8.c) is self hosting. [assembler.ffvm](./assembler.ffvm) is the FFVM assembler written in the compact language of stage 7: a program that reads compact source from the input tape and writes bytecode to the output tape. It is assembled by the C reference assembler, loaded into ROM, and run on the interpreter, so the thing doing the assembling is an ffmpeg filtergraph. The claim the stage makes is an equality: for a given source, the bytecode the machine produces is byte for byte the bytecode the C assembler produces, and this holds when the source is the assembler's own.

The program is 828 characters and assembles to 551 words of bytecode. It cannot use a chain of comparisons to decode mnemonics, because the compact language has twenty six labels in total and a twenty two way branch would spend most of them. Instead it builds a dispatch table in data memory before it starts: for each opcode character it stores one packed value at the address given by that character's code, holding the opcode in the upper bits and a class in the lower three, where the class distinguishes whitespace, a label definition, an instruction with no operand, one with a decimal operand, and one with a label operand. Decoding a character is then a single indirect load and two divisions, and the branch that follows tests the class rather than the character. The table occupies the data cells from 4000 to 4127 and the symbol table the cells from 4400 to 4425, both above the source, which is capped at 3900 characters so the two cannot meet.

The two passes mirror the C assembler. The first walks the source computing instruction widths, so that a label definition records the address it names into the symbol table. The second walks it again emitting bytecode, resolving each label operand out of the symbol table and parsing each decimal operand digit by digit. Because the output tape is dumped a byte per cell and the machine's words are sixteen bits, every emitted word is written as two cells, high byte first, and the reference assembler writes its bytecode the same way so the two artifacts are directly comparable. The instruction set has no call or return, so the emit sequence is written out at each of the two places it is needed rather than shared.

[selfhost.sh](./selfhost.sh) runs the two longer proofs: the same equality for a program carrying an operand wider than a byte, and then the assembler assembling its own source, which is 69671 interpreter steps and runs for some hours. It reports progress every thousand steps. A run that long is worth being able to interrupt. Given a fourth argument the driver treats it as a state file, writing the current frame and step count there every thousand steps and resuming from it if it already exists, so a run killed by a suspended laptop or a closed terminal continues from its last checkpoint rather than starting over. A fifth argument caps the number of steps, which makes the run splittable into sessions: the driver stops at the cap, saves the state and exits with status 2, and the next invocation carries on. The state is written to a temporary name and renamed into place, so a checkpoint interrupted while being written leaves the previous one intact. The files are removed once the program halts. [selfhost.sh](./selfhost.sh) passes a state file, so running it again after an interruption continues the proof.
