...
static void usage(const char *nm) {
	...
	fprintf(stderr, "example: %s p300p500+eh\n", nm);
}
...
int main(int argc, char **argv) {
	...
	char pa[64], pb[64];
	snprintf(pa, sizeof(pa), "/tmp/ffvm7_%d_a.pgm", (int)getpid());
	snprintf(pb, sizeof(pb), "/tmp/ffvm7_%d_b.pgm", (int)getpid());
