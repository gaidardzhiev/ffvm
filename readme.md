...
[Stage 3](./ffvm_stage3.c) is a C program that implements the stored program interpreter and adds control flow. Stage 2 compiled each instruction into its own `geq` stage, which makes the graph a function of the program and rules out jumps to computed addresses. Stage 3 inverts that arrangement. The filter graph becomes a single `geq` expression implementing one interpreter step, and the program moves into the ROM region of the frame as data. Each invocation reads the instruction pointer from the register row, fetches the opcode and its immediate from row 192, decodes, computes the resulting stack, register, flag and IP state, and writes the successor frame. The driver applies the same expression repeatedly until the halt flag appears in the flags register. Nothing in the graph depends on the program being run, so dynamic control flow needs no rewiring: a branch is a write into the IP pixel like any other value. This stage validates the stored-program model end to end. The instruction set at this stage carries over `push`, `pop`, `dup`, `swap`, `add`, `sub`, `mul` and `div` from stage 2 and adds the comparisons `eq`, `lt` and `gt`, the control instructions `jmp`, `jz`, `jnz` and `halt`, and `loadr` and `storer` for the general purpose registers. The register instructions are what make loop verification meaningful, since a loop that cannot accumulate proves only that the instruction pointer moves backwards. Opcodes are grouped by class so the decode cascade can test ranges rather than individual values: control instructions occupy 16 through 19, comparisons 20 through 22, and register access 48 and 49. The assembler resolves labels in two passes. The first walks the token list computing instruction widths to assign an address to every label; the second emits opcodes and immediates, substituting resolved addresses into jump targets. Labels are written with a colon suffix and referenced as the operand of a jump, so a countdown loop is expressed as push:5,loop:,push:1,sub,dup,jnz:loop,emit,halt. Jump operands that parse as numbers are taken as literal addresses, which keeps hand written absolute jumps available. Two properties of the expression evaluator shape the implementation. It provides no `and` or `or` functions, so the conditions in the decode cascade are built from multiplication and `max` over comparison results, which return zero or one. And because the driver invokes ffmpeg once per execution step rather than once per program, the invocation passes `-y`; without it the second step blocks on the interactive overwrite prompt and the run stalls.
